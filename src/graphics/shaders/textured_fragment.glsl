...
const float numerical_zero = 0.001; // 1/pi
const float ambiant_light = 0.1;
...
void main() {
    ...
    float specular;
    if (NL < numerical_zero || NV < numerical_zero || NH < numerical_zero) {
        specular = 0;
    } else {
        float non_roughness = 1 - ubo.roughness;
        float microfacet_distribution_coeff = 1 - non_roughness * NH * NH;
        float visibility_coeff = (NL + sqrt(ubo.roughness + non_roughness * NL * NL)) * (NV + sqrt(ubo.roughness + non_roughness * NV * NV));
        specular = ubo.roughness / (visibility_coeff * microfacet_distribution_coeff * microfacet_distribution_coeff);
    }
    ...
    float albedo_coeff = max(NL, 0.0) + ambiant_light;
    ...
    float colored_coeff = lambertian_diffuse * albedo_coeff * (ubo.metalness * specular * (1 - schlick_coeff) + non_metalness * (1 - fresnel_mix_coeff));
